...
class Rizzo(object):
        ...
        while ea < block.end_ea:
            ...
            # negative values.
                ...
                for n in range(0, len(idaapi.insn_t().ops)):
                    ...
                    if idaapi.getFlags(idaapi.insn_t().ops[n].value) != 0:
                        continue
    ...
    def generate(self):
        ...
        for (ea, string) in self.strings.items():
            ...
            if strhash in signatures.strings.keys():
        ...
        for ea in idautils.Functions():
            ...
            if formal in signatures.formal.keys():
            ...
            if fuzzy in signatures.fuzzy.keys():
            ...
            for (e, f, immediates, c) in blocks:
                for immediate in immediates:
                    if immediate in signatures.immediates.keys():
    ...
    def rename(self, ea, name):
        ...
        if curname.startswith('sub_') and name.split('_')[0] not in {'sub', 'loc', 'unk', 'dword', 'word', 'byte'}:
            ...
            if idc.get_name_ea_simple(name) == idc.BADADDR:
                if idc.set_name(ea, name, ida_name.SN_NOCHECK)	:
